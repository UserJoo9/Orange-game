...
bg_music_sound = pygame.mixer.Sound('bg_music.mp3')
bg_music_sound.set_volume(min(1.0, bg_music_sound.get_volume() / 8))
...
# controller navigate
controller_nav = pygame.font.Font(None, 36).render(f"Controller connecting..", True, (255, 140, 0))
controller_nav_loc = controller_nav.get_rect()
controller_nav_loc.center = width / 2, 30
...
def next_cup():
    global cup_loc, main_object_loc, res_cup, cup, isReady
    pygame.time.delay(200)
    isReady = False
    while cup_loc[0] < width and main_object_loc[0] < width:
    ...
    orange()
    while cup_loc[0] < width / 2 - 200 and main_object_loc[0] < width / 2 - 200:
        cup_loc = cup_loc.move(100, 0)
        main_object_loc = main_object_loc.move(100, 0)
        pygame.display.update()
        pygame.time.delay(50)
...
def active_press():
    ...
    while 1:
        ...
        try:
            ...
            controller_nav = pygame.font.Font(None, 36).render(f"Controller disconnected!", True, (255, 0, 0))
...
while running:
    ...
    still_pressed()
    if not stillPressed and isReady and not rest:
        grabbed = False
        squeeze_sound.stop()
        orange()
        if full_cup:
